...
function Sidebar({ isOpen, toggleSidebar }) {
  ...
  useEffect(() => {
    if (isOpen) {
      setShowContent(false); // Hide content before opening
      const timer = setTimeout(() => {
        setShowContent(true); // Show content after 600ms
      }, 600);
      return () => clearTimeout(timer);
    } else {
      setShowContent(false); // Hide content immediately when closing
      setIsClosing(false);
    }
  }, [isOpen]);
